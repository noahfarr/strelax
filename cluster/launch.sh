#!/usr/bin/env bash
# Submit stremax MinAtar examples to SLURM. Run on the ias login node from the repo root.
# Defaults to the VOGD eta sweep across all MinAtar envs (Breakout skips eta 0.5) on CPU.
# Override via env vars: DEVICE=gpu, EXAMPLE, ETAS, ENVS, SKIP, PARTITION, CPUS, MEM, TIME, WANDB.
set -euo pipefail

export PATH="$HOME/.local/bin:$PATH"

EXAMPLE="${EXAMPLE:-vogd_q_minatar}"
DEVICE="${DEVICE:-cpu}"
WANDB="${WANDB:-1}"

if [ "$DEVICE" = "cpu" ]; then
    PARTITION="${PARTITION:-stud}"
    GRES="${GRES:-}"
    CPUS="${CPUS:-8}"
    MEM="${MEM:-8G}"
    TIME="${TIME:-1-00:00:00}"
    dev_env="export JAX_PLATFORMS=cpu"
else
    PARTITION="${PARTITION:-stud}"
    GRES="${GRES:-gpu:1}"
    CPUS="${CPUS:-4}"
    MEM="${MEM:-16G}"
    TIME="${TIME:-08:00:00}"
    dev_env=""
fi

read -r -a ENVS <<< "${ENVS:-gymnax::Breakout-MinAtar gymnax::Asterix-MinAtar gymnax::Freeway-MinAtar gymnax::SpaceInvaders-MinAtar}"
read -r -a ETAS <<< "${ETAS:-0.25 0.5 1.0}"
SKIP="${SKIP:-gymnax::Breakout-MinAtar:0.5}"

cd "$(dirname "$0")/.."
mkdir -p logs
uv sync

wandb_flag=""
[ "$WANDB" = "1" ] && wandb_flag="--wandb"

submit() {
    name="$1"
    shift
    gres_arg=()
    [ -n "$GRES" ] && gres_arg=(--gres="$GRES")
    sbatch --job-name="$name" \
        --partition="$PARTITION" "${gres_arg[@]}" \
        --cpus-per-task="$CPUS" --mem="$MEM" --time="$TIME" \
        --output="logs/%x-%j.out" <<EOF
#!/usr/bin/env bash
set -euo pipefail
export PATH="\$HOME/.local/bin:\$PATH"
${dev_env}
echo "host=\$(hostname) job=\$SLURM_JOB_ID gpu=\${CUDA_VISIBLE_DEVICES:-none} platforms=\${JAX_PLATFORMS:-default}"
command -v nvidia-smi >/dev/null && nvidia-smi -L || true
uv run --no-sync python "examples/${EXAMPLE}.py" $* ${wandb_flag}
EOF
}

for env_id in "${ENVS[@]}"; do
    short="${env_id#gymnax::}"
    short="${short%-MinAtar}"
    for eta in "${ETAS[@]}"; do
        case " $SKIP " in *" ${env_id}:${eta} "*) echo "skip ${env_id} eta=${eta}"; continue;; esac
        submit "${EXAMPLE}-${short}-eta${eta}" --env-id "$env_id" --eta "$eta"
    done
done
